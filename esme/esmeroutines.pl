...
sub rep
{
    ...
    $dbh->do("INSERT INTO `esme_eventlog` (`le_timestamp`, `le_text`) VALUES (UNIX_TIMESTAMP(), ?  )",
                			 undef, $repmsg );
...
{
   my ($code) = @_;
   return Net::SMPP::status_code->{$code}->{msg}." [".Net::SMPP::status_code->{$code}->{code}."] ($code)";
}
sub connect_transport
{
    ($smpp_t,$err) =
    Net::SMPP->new_transceiver(
    #Net::SMPP->new_reciever(
        $sms_ip,
        port=>$sms_port,
...
sub Send{
	...
	if(length($msgtext1)>254){
		$resp = $smpp_t->submit_sm(
			...
			message_payload =>$msgtext1
			);
		rep("$dest_addr <=L= $src_addr ($msgtext)");
	}else{
		$resp = $smpp_t->submit_sm(
			protocol_id=>0x00,
			service_type=>0x00,
		...
		rep("$dest_addr <=S= $src_addr ($msgtext)");
	}
	if(!$resp || $resp->{status}!=0) {
		$dbh->do("UPDATE `esme_message_out` SET `msgo_lastretry`=UNIX_TIMESTAMP(), `msgo_lasterror`= ?  WHERE `msgo_id`= ?", undef, explain_status($resp->{status}) , $msgo_id );
		...
		if($resp->{status}==11 || $resp->{status}==10){
			...
			$dbh->do("UPDATE `esme_message_out` SET `msgo_sendtime`=UNIX_TIMESTAMP(), `msgo_smsc_id`= ?  WHERE `msgo_id`= ?", undef, $resp->{message_id} , $msgo_id );
		}
		return 0;
	}else{
		$dbh->do("UPDATE `esme_message_out` SET `msgo_sendtime`=UNIX_TIMESTAMP(), `msgo_smsc_id`= ?  WHERE `msgo_id`= ?", undef, $resp->{message_id} , $msgo_id );
...
{
	my ($tx) = @_;
	if ($tx =~ m/^id:([a-z0-9+-]+) sub:([0-9]{1,3}) dlvrd:([0-9]{1,3}) submit date:([0-9]{10}) done date:([0-9]{10}) stat:([a-z]{7})/i) {
		return 1;
	}else{
		return 0;
	}
...
sub processQueue
{
	...
	#return;
	# ��������� �������� ��������� � ���������� ��
    my $sth1 = $dbh->prepare("SELECT  `msgi_id`,`msgi_recvdtime`,`msgi_from`,`msgi_body`,`sn_number`,`msgi_seqnum` FROM `esme_message_in` WHERE  `msgi_processtime`='0' ");
    ...
    if( ('0E0' eq $inresult ) ){
		...
		while(my ($msgi_id,$msgi_recvdtime,$msgi_from,$msgi_body,$sn_number,$msgi_seqnum) = $sth1->fetchrow_array()){
			...
			if ($msgi_body =~ m/^id:([a-z0-9+-]+) sub:([0-9]{1,3}) dlvrd:([0-9]{1,3}) submit date:([0-9]{10}) done date:([0-9]{10}) stat:([a-z]{7})/i) {
				...
				$dbh->do("UPDATE `esme_message_out` SET `msgo_status`='$6' WHERE `msgo_smsc_id`= ?", undef, $1);
				$dbh->do("UPDATE `esme_message_in` SET `msgi_isreport`='1' WHERE `msgi_id`= ?", undef, $msgi_id);
			}else{ #���������� ����������
				ruleProcessor(%sms);
			}
			$dbh->do("UPDATE `esme_message_in` SET `msgi_processtime`=UNIX_TIMESTAMP() WHERE `msgi_id`= ?", undef, $msgi_id);
		}
	...
	#rep("Checking outbox...");
	my $sth = $dbh->prepare("SELECT `msgo_id`,`msgo_body`,`msgo_to`,`msgo_from`,`msgo_reqstatus` FROM `esme_message_out`
								WHERE  `msgo_sendtime`='0' AND (`msgo_lastretry`='0' OR `msgo_lastretry`<UNIX_TIMESTAMP()-300) AND `msgo_gentime`>UNIX_TIMESTAMP()-86400*2 ");
...
sub queueMsg
{
	...
	$dbh->do("INSERT INTO `esme_message_out` (`msgo_body`,`msgo_gentime`,`msgo_from`,`msgo_to`,`msgo_reqstatus`) VALUES ( ?, UNIX_TIMESTAMP(), ?, ? )",
...
sub inboxQueueMsg
{
	...
	$dbh->do("INSERT INTO `esme_message_in` (`msgi_text`,`msgi_recvdtime`,`msgi_from`,`dest_addr`) ".
...
sub mainloop(){
	...
	#rep("about to can_read()");
		...
		#print "got cmd = ".$pdu->{cmd}."\n";
			...
			switch($pdu->{cmd}){
			case Net::SMPP::CMD_deliver_sm {
				$smpp_t->deliver_sm_resp(seq=>$pdu->{seq},message_id=>"");
				my $sms = &decode($pdu->{data});
				my $isr = isReport($sms->{short_message} or "");
				rep( ($sms->{source_addr} or "")." ==> \"".($sms->{short_message} or "")."\"")
					if $isr ne 1;
				last if !$sms->{source_addr};
				#register this SMS in the inbox
				$dbh->do("INSERT INTO `esme_message_in` (`msgi_recvdtime`,`msgi_from`,`msgi_body`,`sn_number`,`msgi_seqnum`)".
							" VALUES (UNIX_TIMESTAMP(), '".$sms->{source_addr}."' ,".$dbh->quote($sms->{short_message})." ,'".$sms->{destination_addr}."' , '".$pdu->{seq}."' )" );
			}
			case Net::SMPP::CMD_unbind {
				rep("Got unbind cmd. Breaking loop");

				$smpp_t->unbind_resp() if $died!=1;
				$died = 1;
				return;
			}
			case Net::SMPP::CMD_unbind_resp {
				rep("Got unbind_resp cmd. Breaking loop");
				$died = 1;
				return;
			}
			case Net::SMPP::CMD_enquire_link{
				rep("Got CMD_enquire_link");
				$smpp_t->enquire_link_resp(seq=>$pdu->{seq});
			}
			case Net::SMPP::CMD_enquire_link_resp{
				rep("Got CMD_enquire_link_resp");
				#$smpp_t->enquire_link_resp();
			}
			case Net::SMPP::CMD_alert_notification{
...
sub ruleProcessor{
    ...
    my $rq = $dbh->prepare("SELECT `rule_id`, `rule_name` FROM `esme_rules` WHERE `rule_enabled`!='0' ORDER BY `rule_order` ASC");
    ...
    if( ('0E0' eq $rulesres ) ){
        ...
        while(my ($rule_id,$rule_name) = $rq->fetchrow_array()){
			...
			my $rule_true = 1;
            my $rcq = $dbh->prepare("SELECT `rc_key` , `rc_relativity` , `rc_val`  FROM `esme_rule_conditions` WHERE `rule_id`='$rule_id' ORDER BY `rc_id` ASC");
...
#					    case "CurrentDate" 		{ }
			...
			if($rule_true == 1){
				...
				#fetch actions
				my $raq = $dbh->prepare("SELECT `ra_action` , `ra_atribute`, `ra_id`  FROM `esme_rule_actions` WHERE `rule_id`='$rule_id' ORDER BY `ra_id` ASC");
            	...
            	while(my ($ra_action, $ra_atribute,$ra_id ) = $raq->fetchrow_array()){
	                switch($ra_action){
	                    ...
	                    case "ExternalFilter" {
                            ...
                            my $EFerr = '';
							my $efq = $dbh->prepare("SELECT `pl_id`, `pl_path`, `pl_isactive`, `pl_timeout`  FROM `esme_plugins` WHERE `pl_name`='$ra_atribute'");

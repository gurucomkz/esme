...
#
# $Id: SMPPSSL.pm,v 0.1 2010/27/02 19:04:36 sampo Exp $
...
sub explain_status {
    ...
    return sprintf("%s (%s=0x%08X)",
		   Net::SMPPSSL::status_code->{${*$me}{status}}->{msg},
		   Net::SMPPSSL::status_code->{${*$me}{status}}->{code},
		   ${*$me}{status});
}
...
sub explain_cmd {
    my $me = shift;
    my $cmd = Net::SMPPSSL::pdu_tab->{${*$me}{cmd}}
...
sub read_pdu {
    ...
    return bless $pdu => 'Net::SMPPSSL::PDU';
}
...
Net::SMPPSSL - pure Perl implementation of SMPP 3.4 over TCP

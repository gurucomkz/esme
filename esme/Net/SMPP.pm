...
use constant param_tab => {
    ...
    #     useful
	0x1400 =>   { name => 'Service_Code', technology => 'GSM', },
	#0x1401 =>   { name => 'XMessagePayload', technology => 'GSM', },
	0x1775 => 	{ name => 'Service_Id', technology => 'GSM', },
	0x1776 => 	{ name => 'Transaction_Id', technology => 'GSM', },
	#0x0A50 => { name => 'Service_Code', technology => 'GSM', },
	#0x0A51 => { name => 'Transaction_Id', technology => 'GSM', },
};
...
sub decode_optional_params {
	...
	if (defined param_tab->{$tag}) {
	    ...
	    print "Unknown tag (offset $offset): $tag, len=".length($val).", val=`$val'\n";
		#my $unkname = "unknown$tag";
		$pdu->{$tag} = $val; 
	}
	$offset += 4 + length($val);
    }
